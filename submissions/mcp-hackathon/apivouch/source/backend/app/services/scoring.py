...
def compute_score(analyses: list[dict], issues: list[dict], test_results: list[dict] | None = None) -> dict[str, Any]:
    ...
    # Reliability is evidence-weighted. Skipped/blocked endpoints do not receive
    # credit, so a high score always corresponds to callable evidence.
    if test_results:
        passed = sum(1 for t in test_results if t.get("status") == "passed")
        warnings = sum(1 for t in test_results if t.get("status") == "warning")
        reliability = round((passed + warnings * 0.4) / max(1, len(analyses)) * WEIGHTS["reliability"])
        reliability = min(WEIGHTS["reliability"], reliability)
    else:
        reliability = 5
    ...
    return {"overall": total,
            ...
            "evidence": {"endpoints_analyzed": len(analyses), "endpoints_probed": len(test_results or []),
                         "high_or_critical_findings": crit},
